...
def get_pod(metadata):
    ...
    metadata['pod'] = pod.items[pods.index(
        metadata['pod_name'])].spec.containers[0].env
    return metadata
...
def main():
    ...
    datapoints = [
        ["Data points", "Values"],
        [f"Environment variable in {a['context']} » {a['namespace']} » {a['pod_name']}", str(
            len(a_env.items()))],
        [f"Environment variable in {b['context']} » {b['namespace']} » {b['pod_name']}", str(
            len(b_env.items()))],
        [f"Conflicting environment key(s) count in {a['pod_name']}", str(
            len(list(a_env.keys() - b_env.keys())))],
        [f"Missing keys in {a['pod_name']}", str(
            list(a_env.keys() - b_env.keys()))],
        [f"Conflicting environment key(s) count in {b['pod_name']}", str(
            len(list(b_env.keys() - a_env.keys())))],
        [f"Missing keys in {b['pod_name']}", str(
            list(b_env.keys() - a_env.keys()))]
    ]
    print(AsciiTable(datapoints).table)

    compare_var_table = []
    compare_var_table.append(
        ['Key', f"{a['pod_name'].strip()}", f"{b['pod_name'].strip()}"])
